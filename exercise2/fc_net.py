...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # hidden,cache_x = relu_forward(hidden)
        ...
        # # backpropagation
        # dx2,dw2,db2 = affine_backward(dscore,cache_2)
        # dx1 = relu_backward(dx2,cache_x)
        # dx,dw1,db1 = affine_backward(dx1,cache_1)

        # # dx1, dw2, db2 = affine_backward(dscore, cache_2)
        # # _, dw1, db1 = affine_relu_backward(dx1, cache_1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(self.num_layers):
            wi = weight_scale * np.random.randn(dims[i],dims[i+1])
            bi = np.zeros(dims[i+1])
            self.params.update({'W'+str(i+1):wi,
                                'b'+str(i+1):bi})
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            x, hidden['cache'+str(i)] = affine_forward(x, w, b)
        ...
        for i in range(self.num_layers)[::-1]:
            ...
            if i == self.num_layers:
                dout = dscore
                dx,dw,db = affine_backward(dout,hidden['cache'+str(i)])
                
            else:
                dout = dx
                
                dx = relu_backward(dout,hidden['x'+str(i)])
