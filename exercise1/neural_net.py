...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        ########################################################################
        XW1 = X.dot(W1) + b1
        #ReLU activation between layer, speed up training: make socre<0 has gradient=0
        # same matrix size
        y1 = np.maximum(0,XW1)  
        scores = y1.dot(W2) + b2
        ...
        score_i = exp_scores / sum_scores
        loss = np.sum(-np.log(score_i[np.arange(N),y]))
        ...
        # of same size                                                         #
        ########################################################################
        # dL/dw = (score-label y)*input X    label y = 1 for correct, =0 for incorrect
        dscores = score_i
        dscores[range(N),y] -= 1
        dscores /= N

        grads['W2'] = np.dot(y1.T, dscores)
...
##############  
    ...
    def predict(self, X):
        ...
        output_score = hidden_score.dot(self.params['W2'])+self.params['b2']
        y_pred = np.argmax(output_score, axis = 1)
        ########################################################################
        #                              END OF YOUR CODE                        #
        ########################################################################
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    ############################################################################
    numiter = 5000
    learning_rates = np.geomspace(2e-03, 3e-03, num=2)
    regularization_strengths = np.geomspace(1.4e-05, 1.8e-05, num=2)
    delay = [0.95,0.99]
    input_size = 32 * 32 * 3
    hidden_size = 50
    num_classes = 10
    ...
    for lr in learning_rates:
        for rs in regularization_strengths:
            for dy in delay:
                classifier = TwoLayerNet(input_size, hidden_size, num_classes)
                classifier.train(X_train, y_train,X_val, y_val, learning_rate=lr, reg=rs,num_iters=numiter, learning_rate_decay=dy)
                val_pred = classifier.predict(X_val)
                vali_accu = np.mean(val_pred == y_val)
                results[(lr,rs,dy)]=(vali_accu)

                if best_val < vali_accu:
                    best_val = vali_accu
                    best_net = classifier

    for (lr, rs,dy) in sorted(results):
        val_accuracy = results[(lr, rs,dy)]
        print('lr %e； reg %e；  learning rate delay %e； val accuracy: %f' % (
          lr, rs, dy, val_accuracy))
